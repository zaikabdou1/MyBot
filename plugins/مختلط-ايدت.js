...
module.exports = {
  command: 'ارثر',
  description: 'يبعت فيديوهات مختلطة عشوائية + ردة فعل 🎞 لأي حد',
  category: 'edit',

  async execute(sock, msg) {
    try {
      const allowedUsers = ['213773231685', '104806312050733', '227552333414482'];
      const sender = msg.key.participant || msg.key.remoteJid;
      const cleanSender = sender.replace(/[^0-9]/g, '');

      if (!allowedUsers.includes(cleanSender)) {
        return await sock.sendMessage(msg.key.remoteJid, {
          text: '🚫 هذا الأمر حصري للأرقام المصرح لها فقط'
        }, { quoted: msg });
      }
      ...
      await sock.sendMessage(chatJid, {
        text: `🫦 فيديو مختلط عشوائي قادم! استعد 😏`,
      });
      ...
      console.error('خطأ في أمر مختلط-ايدت:', err);
