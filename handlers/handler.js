...
// 🧱 فحص الجروبات المحظورة (يُقرأ في كل مرة لتحديث مباشر)
if (message.key.remoteJid.endsWith('@g.us')) {
    try {
        const blockedPath = path.join(__dirname, '../data/blockedGroups.json');
        if (fs.existsSync(blockedPath)) {
            // اقرأ الملف في كل رسالة
            const blockedGroups = JSON.parse(fs.readFileSync(blockedPath, 'utf8') || '[]');

            if (blockedGroups.includes(message.key.remoteJid)) {
                const allowedCommands = ['جروبي-متبند', 'فك-بان'];
                if (!allowedCommands.includes(commandWithoutPrefix)) {
                    logger.warn(`🚫 تجاهل رسالة من جروب محظور: ${message.key.remoteJid}`);
                    return;
                }
            }
        }
    } catch (err) {
        logger.error('خطأ أثناء فحص الجروبات المحظورة:', err.message);
    }
}
